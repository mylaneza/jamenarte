...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "Stroopium";
    ...
    public Vector<Paso> getPasosDeLeccion(long id){
        ...
        // Define a projection that specifies which columns from the database
        // you will actually use after this query.
        Vector<Paso> pasos = new Vector<Paso>();
        for( int i = 0; i < stepsIds.length ; i++){
            pasos.add(getPaso(stepsIds[i]));
        }
    ...
    public void insertaPasoLeccion(long leccionId,long pasoId){
        ...
        long id = db.insert( DBContract.LessonStepList.TABLE, null, values);
        close();
    }
    ...
    public void exportar(){
        ...
        File dir = new File(Environment.getExternalStorageDirectory() + "/jamenarte");
        if(!dir.exists())
            dir.mkdir();
        File f =new File(Environment.getExternalStorageDirectory() + "/jamenarte", "db.txt");
        try {
